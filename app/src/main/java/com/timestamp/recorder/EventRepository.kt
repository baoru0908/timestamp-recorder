...
object EventColors {
     ...
     *   同一列里黑白混排不统一，被否掉）；
     * - **仅底色极亮时才转近黑** —— 只兜住白字真正会"消失"的那一档，
     *   例如纯白 #FFFFFF（白字 1.00:1）、明黄 #FDD835（1.40:1）。
     *
     * 判定用**相对亮度 > 0.60**（而不是逐色比对比度）。
     *
     * ⚠️ **2026-09-21 主人拍板「解读 B」：阈值上调 0.45 → 0.60，白字一致性优先。**
     * 语义 = **白字优先 / 一致性优先**：更少底色转深色字，白字覆盖更多场景；
     * 代价是**琥珀 #F9A825 由近黑改回白字**（8.67:1 → 1.97:1），浅底可读性下降，
     * 主人已知并接受。（另一方向「解读 A = 下调阈值让更多浅底转深色字」已被否决，
     * **不要改回去**。）
     *
     * 实测结论（23 色色板，按本文件 [relativeLuminance] / [contrastWithWhite] 复算）：
     * - 新阈值下 **22 色保持白字**，仅明黄 #FDD835（亮度 0.7025）转近黑；
     *   琥珀 #F9A825（0.4828）由旧规则的近黑**改回白字**，是本次唯一翻转的色；
     * - 保持白字的 22 色里 **6 色白字对比度 < 3:1**：琥珀 1.97、亮青 2.30、橙 2.37、
     *   浅蓝 2.65、青 2.74、薄荷 **2.9985**（全部未达 3:1，勿把 2.9985 当成达标）；
     *   若按 WCAG AA 正文线 4.5:1 计，**不达标的共 13 色**（另含深橙 3.08、草绿 3.18、
     *   绿 3.30、蓝 3.68、玫粉 3.76、红 4.23、青绿 4.32）——
     *   引用"6 色"时勿被误读为 AA 不达标的全部；
     * - **回归面 = 仅琥珀 1 色**：只有亮度落在旧 0.45 与新 0.60 之间（琥珀 0.4828）的色
     *   会翻转，其余 22 色新旧判定完全一致（其中亮青/橙/浅蓝/青/薄荷 5 色本就是
     *   "白字 + 对比偏低"的历史状态，**非本次引入**）；
     * - 0.50 ~ 0.70 对 23 色的判定结果**完全相同**（色板亮度在 0.4828 与 0.7025 之间
     *   存在空档），取 0.60 是主人建议的起点，两侧各留余量；
     * - ⚠️ **2026-09-21 v4.7.0 起本行结论已变**：自定义取色器**不再**用 [maxValueForWhite]
     *   收口（面板可选到纯白），所以"取色器产出色都是白字"不再成立 ——
     *   纯白 / 高亮浅色底上的文字会由 [onColor] 转成 [NEAR_BLACK]，与 [LIGHT_BACKGROUND_LUMINANCE]
     *   的 0.60 阈值一致。这是"要完整亮度范围"的必然代价（用户已拍板接受）。
     */
    /** 底色"极亮"的判定线（WCAG 相对亮度）：只有白字会消失的那一档才越线 */
    const val LIGHT_BACKGROUND_LUMINANCE = 0.60
     ...
     *
     * ⚠️ **2026-09-21（v4.7.0）起本函数当前未被任何地方使用，保留备用**：
     * 取色器的 SV 面板已放开为完整 0~100% 明度（用户要求可选纯白与高亮浅色），
     * 不再按这个上限收口。将来若要恢复"只给白字读得清的颜色"，可直接复用本函数。
     *
     * 原用途：把 SV 面板的**纵向范围**按这个上限压下来，于是面板里每一个点都是
     * "白字读得清"的颜色 —— 从根上避免挑出一个白色/极浅色事件，导致卡片上的白点、
     * 白色药丸、白色文字全部糊在一起（2026-09-21 真机实测：#FFFFFF 事件在深色卡片上就是一片白）。
     *
     * 相对亮度对 v 单调递增，所以用二分。
     *
     ...
     *
     * ⚠️ **2026-09-21（v4.7.0）起本函数当前未被任何地方使用，保留备用**：
     * 取色器已不再夹取初始色、也不再夹取落库颜色（否则选不出纯白与高亮浅色）。
     * 事件色前景的可读性改由 [onColor]（阈值 [LIGHT_BACKGROUND_LUMINANCE]）在显示时兜底。
     */
