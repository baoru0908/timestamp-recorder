...
## 0. 交付流程（2026-09-21 用户要求：已取消独立复核）

> ⚠️ **本版（v4.7.0）起生效**：用户明确要求精简流程，**取消独立的 QA 复核、真机取证与额外自验收环节**。

- **新的最短路径**：改完功能 → `./gradlew.bat assembleDebug` **编译通过** → **直接发版**。
  「本机编译通过」即本轮质量闸门，**不再**要求独立 QA 复核 / 真机截图取证 / 双机型回归 / 额外测试轮。
- **仍然强制、一字未松**（取消的只是"独立复核"，不是发布本身）：
  1. **发版三件套** —— 签名包 + tag + GitHub Release（逐条实测，见 §5）；
  2. **文档三同步** —— `docs/release-notes/vX.Y.Z.md` + `DEVELOPMENT.md` + `README.md`（见 §2）；
  3. **版本号语义正确**、`versionCode` 恰好 +1（见 §1）。
- 历史：v4.6.0 / v4.6.1 曾走「独立复核 + 真机回归」流程；**自 v4.7.0 起该环节按用户要求移除**，
  §5 清单里对应的两行已标注"已取消"，无需再逐条勾。其余规则全部照旧。
...
- [x] ~~真机回归：开 / 关高级材质两种形态布局一致，App 可正常启动~~ → **按用户要求已取消（2026-09-21）**
- [x] ~~双机型（小米 HyperOS / realme UI）至少一台完成回归~~ → **按用户要求已取消（2026-09-21）**
