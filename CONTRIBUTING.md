...
> 📌 **但「质量门」不是一刀切**：验证强度按**改动分级**（§0.2）决定 ——
> L0 / L1 只走 S2；**L2 大型改动额外保留「一次」独立 QA**（只做一次、必须交凭证）。
> 「不做复核」针对的是**重复复核**，**不是**「大改动的那一次验证」。
...
> 这是判断「要不要上独立 QA」的**唯一依据**。判定速记：**「这次改动会不会碰到别人」** ——
> 会碰到（共享代码 / 数据契约 / 既有语义）就是 **L2**。

| 档位 | 判定（命中任一即为该档） | 流程强度 |
|:---|:---|:---|
| **L0 文案 / 注释** | 无行为变化 | 直接提交（不触发 S1/S2/S3） |
| **L1 单点修复** | 1 个文件且影响面已算清 | S1 构建 + S2 推真机 |
| **L2 大型改动** | 命中下方任一条件 | **S1 + S2 + 独立 QA 复核（只做一次）** |

**L2「大型」判定条件 —— 命中任意一条即算：**

1. **规模**：≥3 个文件，或 ≥100 行实质逻辑
2. **共享性**：触碰被多处引用的常量 / 工具类 / 基类 / 数据层（**影响面 = 全部调用点**）
3. **数据契约**：改动存储格式、备份格式、导入导出、协议字段（**不可逆风险**）
4. **语义变更**：不是新增，而是**修改既有行为**的含义
5. **并发与生命周期**：线程、回调、资源（socket / FD / 内存）、启停时序
6. **跨环境**：依赖 ROM / 系统版本 / 屏幕密度 / 刷新率
7. **无可自动化验证**：纯手感、视觉、无障碍（只能靠真机 + 人眼）

**L2 的 QA 只做一次**，且必须交付可核对凭证：
- **影响面清单**：这次改到了哪些调用点
- **回归结论**：旧功能是否受影响
- **必要时附对比数据**：对比度表 / 像素采样 RGB / 帧耗时
- **不做「再来一轮看看」**；失败只重跑失败项（承接 §0.4 的流程禁令）

> 判定速记：**「这次改动会不会碰到别人」** —— 会碰到（共享代码 / 数据 / 既有语义）就是 L2。

#### 「不设独立 QA 角色」的边界（与 §0.4 流程禁令不冲突）

- **什么时候不 QA**：L0 / L1，以及**任何档位的「重复复核」** —— 不设「第二双眼睛」，同一项只验一次。
- **什么时候必须 QA**：**仅 L2 大型改动**，**且只做一次**，必须交可核对凭证（影响面清单 + 回归结论）。

一句话：**「不设独立 QA 角色」= 不设常驻 / 重复的复核环节；「L2 大型改动」仍保留一次独立 QA，
只做一次、且必须交凭证** —— 砍掉的是「反复验证」，不是「大改动的那一次验证」。
...
1. **不设独立 QA 复核角色** —— 取消「第二双眼睛」的**常驻 / 重复环节**。
   ⚠️ **唯一例外：L2 大型改动保留一次独立 QA**（只做一次、必须交凭证，见 §0.2 的边界说明）。
...
- [ ] **改动分级已判定（L0 / L1 / L2）；若为 L2，已附独立 QA 凭证（影响面清单 + 回归结论）**（§0.2 改动分级）
- [ ] 版本号语义正确、versionCode 恰好 +1
- [ ] release-notes / DEVELOPMENT.md / README 三处已更新并推送
- [ ] **真机回归（S2 必经，单机型即可）**：`adb install -r` 成功 + App 启动无崩溃 + 本次改动功能走通，**留有 1~3 张截图**（§0.3）
